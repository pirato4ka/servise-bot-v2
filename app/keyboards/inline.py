...
def services_list_kb(services):
    kb = []
    for s in services:
        kb.append([InlineKeyboardButton(text=f"{'🟢' if s['is_active'] else '🔴'} {s['title']}", callback_data=f"svc:view:{s['id']}")])
...
def user_invoice_kb(bot_invoice_url: str, crypto_invoice_id: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Оплатити через CryptoBot", url=bot_invoice_url)],
        [InlineKeyboardButton(text="🔄 Перевірити оплату", callback_data=f"checkpay:{crypto_invoice_id}")]
    ])
